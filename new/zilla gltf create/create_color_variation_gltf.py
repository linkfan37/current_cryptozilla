...
def creategltfs( gltf_in = 'thirstplace.gltf'):
    ...
    for line_num, line in enumerate(uri_reader.readlines()):
        ...
        if line_num == 2 : Bodyuri=line
        if line_num == 4 : head_top_uri=line
        if line_num == 6 : mouthuri=line
        if line_num == 12 : left_arm_1=line
        if line_num == 14 : left_arm_2=line
        if line_num == 16 : left_arm_3=line
        if line_num == 18 : right_arm_1=line
        if line_num == 20 : right_arm_2=line
        if line_num == 22 : right_arm_3=line
        if line_num == 24 : left_leg_1=line
        if line_num == 26 : left_leg_2=line
        if line_num == 28 : left_leg_3=line
        if line_num == 30 : right_leg_1=line
        if line_num == 32 : right_leg_2=line
        if line_num == 34 : right_leg_3=line
        if line_num == 36 : tail_1=line
        if line_num == 38 : tail_2=line
        if line_num == 40 : tail_3=line
        if line_num == 42 : tail_4=line
        if line_num == 3 : shade_body=line
        if line_num == 5 : shade_head=line
    ...
    with alive_bar(len(skins),force_tty=True) as bar:
        for skin in skins:
            f = open('/home/flo/projekt/new/zilla gltf create/gltf in/'+gltf_in, 'r')
            f2 = open('/home/flo/projekt/new/zilla gltf create/gltf out/'+color[i]+"_"+gltf_name+'.gltf', 'w')
            while True:
                line = f.readline()
                with open('/home/flo/projekt/new/zilla gltf create/skins/'+skin) as json_file:
